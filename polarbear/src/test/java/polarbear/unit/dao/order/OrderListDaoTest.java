...
public class OrderListDaoTest extends AbstractTransactionalJUnit4SpringContextTests {
    ...
    public void testQueryOrderList() throws DaoException {
        List<OrderList> orderLists = orderListDao.findByNamedQuery("queryListByOrderId", createUser1_2ProductUnpayOrder1());
    ...
    public void testbatchUpdateOrderState() throws DaoException {
        final ORDER_STATE STATE = CANCLE;
        orderListDao.executeUpdate("batchUpdateStateByOrderId", STATE.value(), createUser1_2ProductUnpayOrder1());
        List<OrderList> orderLists = jdbcTemplate.query("select * from orderlist where order_id = ?", new Object[] { createUser1_2ProductUnpayOrder1().getId() },
                BeanPropertyRowMapper.newInstance(OrderList.class));
        assertThat("订单列表商品", orderLists.size(), equalTo(BUY_PRODUCTS.length));
        assertOrderListState(orderLists, STATE);
    }

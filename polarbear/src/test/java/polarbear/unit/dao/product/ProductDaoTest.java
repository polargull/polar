...
public class ProductDaoTest extends AbstractTransactionalJUnit4SpringContextTests {
    ...
    public void queryProductManagerList() throws DaoException {
        ...
        PageList<Product> pageList = productDao.findPageListByDynamicCondition(Product.class, hqlCondition, 1, 10);
        assertThat(pageList.getList().size(), equalTo(3));
        assertThat(pageList.getList().get(0).getName(), equalTo(PRODUCT_NAME + 1));
        assertThat(pageList.getList().get(1).getName(), equalTo(PRODUCT_NAME + 2));
        assertThat(pageList.getList().get(2).getName(), equalTo(PRODUCT_NAME + 3));
